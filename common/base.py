...
class Trainer(Base):
    ...
    def _make_batch_generator(self):
        ...
        if cfg.stage == 'exer':
            self.class_num = self.trainset.exer_num
        elif cfg.stage == 'attr':
            self.class_num = self.trainset.attr_num
        self.joint_num = self.trainset.joint_num

    def _make_model(self):
        # prepare network
        self.logger.info("Creating graph and optimizer...")
        model = get_model(self.class_num, self.joint_num, 'train')
...
class Tester(Base):
    ...
    def _make_batch_generator(self):
        ...
        self.batch_generator = batch_generator
        if cfg.stage == 'exer':
            self.class_num = self.testset.exer_num
        elif cfg.stage == 'attr':
            self.class_num = self.testset.attr_num
    ...
    def _make_model(self):
        ...
        model = get_model(self.class_num, self.joint_num, 'test')
        model = DataParallel(model).cuda()
